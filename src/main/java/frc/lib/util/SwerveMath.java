...
public class SwerveMath {
  public static Translation2d normalizeWheelAccel(
      Translation2d curTranslationSpeeds, Translation2d lastTranslationSpeeds) {
    ...
    normalizedSpeeds =
        new Translation2d(
            Math.min(forwardLimit.getX(), skidLimit.getX()),
            Math.min(forwardLimit.getY(), skidLimit.getY()));
    return normalizedSpeeds;
  }

  private static Translation2d forwardAccelLimit(
      Translation2d curSpeeds, Translation2d lastSpeeds) {
    ...
    Translation2d forwardProjection =
        new Translation2d(
            angleDiff.getCos() * Math.hypot(curSpeeds.getX(), curSpeeds.getY()), forwardAngle);
    Translation2d remainder = curSpeeds.minus(forwardProjection);

    double forwardDiff =
        Math.hypot(forwardProjection.getX(), forwardProjection.getY())
            - Math.hypot(lastSpeeds.getX(), lastSpeeds.getY());
    double maxAccel =
        MAX_FORWARD_ACCEL * (1 - Math.hypot(lastSpeeds.getX(), lastSpeeds.getY()) / MAX_SPEED);
