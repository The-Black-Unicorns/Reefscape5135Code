...
public class ArmSubsystem extends SubsystemBase {
  ...
  public void setIdleModeBreak() {
    ...
    armMotorL.configure(
        armConfigL, ResetMode.kResetSafeParameters, PersistMode.kNoPersistParameters);
    armMotorR.configure(armConfigR, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);
  }
  ...
  public void setArmAngle(DoubleSupplier targetAngleDegrees) {
    ...
    double ffVoltage =
        armFeedforward.calculate(
            armEncoder.getPosition() * Math.PI / 180.0 - ARM_NORMALIZE_OFFSET * Math.PI / 180.0,
            armEncoder.getVelocity() * Math.PI / 180.0);
    double pidVoltage =
        armPIDController.calculate(armEncoder.getPosition(), targetAngleDegrees.getAsDouble());
  ...
  public void setArmManualSpeed(double speed) {
    ...
    double ffVoltage =
        armFeedforward.calculate(
            armEncoder.getPosition() * Math.PI / 180.0, armEncoder.getVelocity() * Math.PI / 180.0);
  ...
  public void periodic() {
    ...
    // SmartDashboard.putNumber("Arm/pidVoltage",
    // armPIDController.calculate(armEncoder.getPosition(), 60));
  ...
  public void testPeriodic() {
    ...
    currentArmTargetAngle =
        SmartDashboard.getNumber("Arm/desiredAngle", Constants.Arm.ARM_MID_ANGLE);
    ...
    if (newKP != KP || newKI != KI || newKD != KD) {
      ...
      armMotorR.configure(
          armConfigR, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);
      armMotorL.configure(
          armConfigL, ResetMode.kResetSafeParameters, PersistMode.kNoPersistParameters);
    }
