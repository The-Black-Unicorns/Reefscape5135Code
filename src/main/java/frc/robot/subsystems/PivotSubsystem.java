...
//         pivotPIDController = new ProfiledPIDController(PIVOT_MOTOR_KP, PIVOT_MOTOR_KI,
// PIVOT_MOTOR_KD,
//             new TrapezoidProfile.Constraints(MAX_PIVOT_DEGREES_PER_SECOND,
// MAX_PIVOT_DEGREES_PER_SECOND_SQUARED));
...
//       return new TrapezoidProfile.State(pivotAbsoluteEncoder.getPosition(),
// pivotAbsoluteEncoder.getVelocity());
//   }
...
//         double ffVoltage =
// pivotFeedforwardController.calculate(pivotAbsoluteEncoder.getPosition()* Math.PI /
//         180.0 - PIVOT_NORMALIZE_OFFSET* Math.PI / 180.0,
//                                                     pivotAbsoluteEncoder.getVelocity()* Math.PI /
// 180.0);
...
//     double ffVoltage = pivotFeedforwardController.calculate(pivotAbsoluteEncoder.getPosition()*
// Math.PI /
//     180.0 - PIVOT_NORMALIZE_OFFSET* Math.PI / 180.0,
//                                                 pivotAbsoluteEncoder.getVelocity()* Math.PI /
// 180.0);
...
public class PivotSubsystem extends SubsystemBase {
  ...
  public PivotSubsystem() {
    ...
    pivotConfig.absoluteEncoder.zeroOffset((256) / 360.0);
    ...
    pivotFeedforward =
        new ArmFeedforward(
            PivotConstants.PIVOT_MOTOR_KS,
            PivotConstants.PIVOT_MOTOR_KG,
            PivotConstants.PIVOT_MOTOR_KV);
    ...
    pivotPIDController =
        new ProfiledPIDController(
            ...
            new TrapezoidProfile.Constraints(
                MAX_PIVOT_DEGREES_PER_SECOND, MAX_PIVOT_DEGREES_PER_SECOND_SQUARED));
  ...
  public void setIdleModeBreak() {
    ...
    pivotMotor.configure(
        pivotConfig, ResetMode.kResetSafeParameters, PersistMode.kNoPersistParameters);
  }
  ...
  public void setPivotAngle(DoubleSupplier targetAngleDegrees) {
    ...
    double ffVoltage =
        pivotFeedforward.calculate(
            pivotEncoder.getPosition() * Math.PI / 180.0 - PIVOT_NORMALIZE_OFFSET * Math.PI / 180.0,
            pivotEncoder.getVelocity() * Math.PI / 180.0);
    double pidVoltage =
        pivotPIDController.calculate(pivotEncoder.getPosition(), targetAngleDegrees.getAsDouble());
  ...
  public void setPivotManualSpeed(double speed) {
    ...
    double ffVoltage =
        pivotFeedforward.calculate(
            (pivotEncoder.getPosition() - PIVOT_NORMALIZE_OFFSET) * Math.PI / 180.0,
            pivotEncoder.getVelocity() * Math.PI / 180.0);
  ...
  public void periodic() {
    double ffVoltage =
        pivotFeedforward.calculate(
            pivotEncoder.getPosition() * Math.PI / 180.0,
            pivotEncoder.getVelocity() * Math.PI / 180.0);
    // double pidVoltage = armPIDController.calculate(armEncoder.getPosition(), 20.6);
    // SmartDashboard.putNumber("Pivot/pidVoltage",
    // pivotPIDController.calculate(pivotEncoder.getPosition(),90));
  ...
  public void testPeriodic() {
    ...
    currentPivotTargetAngle =
        SmartDashboard.getNumber("Pivot/desiredPivot", Constants.Arm.ARM_MID_ANGLE);
    // if(newKP != KP || newKI != KI || newKD != KD){
    //     KP = newKP;
    //     KI = newKI;
    //     KD = newKD;
    ...
    //     pivotMotor.configure(pivotConfig, ResetMode.kResetSafeParameters,
    // PersistMode.kPersistParameters);
