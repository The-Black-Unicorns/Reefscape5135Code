...
public class SwerveSubsystem extends SubsystemBase {
  ...
  public SwerveSubsystem(File directory) {
    ...
    autoPose =
        NetworkTableInstance.getDefault().getStructTopic("Auto Pose", Pose2d.struct).publish();
    ...
    try {
      swerveDrive =
          new SwerveParser(directory)
              .createSwerveDrive(
                  MAX_SPEED, new Pose2d(new Translation2d(15, 4), Rotation2d.fromDegrees(0)));
    } catch (Exception e) {
  ...
  public void periodic() {
    ...
    // mod.getAbsolutePosition());
    //       SmartDashboard.putNumber("Mod " + mod.moduleNumber + " Angle",
    // mod.getAbsoluteEncoder().getAbsolutePosition());
    ...
    updateLimelightReading(
        () -> swerveDrive.getPose().getRotation().getDegrees(),
        () -> (swerveDrive.getFieldVelocity().omegaRadiansPerSecond * 180.0 / Math.PI));
  ...
  public void drive(
    ...
    // new Translation2d(swerveDrive.getFieldVelocity().vxMetersPerSecond,
  ...
  public Command driveCommandForDriver(
      DoubleSupplier xSpeed,
      DoubleSupplier ySpeed,
      DoubleSupplier angularSpeed,
      BooleanSupplier isFieldOriented,
      DoubleSupplier speedExponent) {
    // int invertInputs = isRedAlliance() ? -1 : 1;
        ...
        () ->
            drive(
                new Translation2d(
                    ...
                    .times(MAX_SPEED)
                    .times(speedExponent.getAsDouble() * Math.abs(speedExponent.getAsDouble())),
  ...
  public Command driveConstantSpeed(
      double x, double y, double rotations, double time, boolean isFieldOriented) {
    return new RunCommand(
            () -> drive(new Translation2d(x, y), rotations, isFieldOriented, true), this)
        .withTimeout(time)
  ...
  //             .toSwerveModuleStates(ChassisSpeeds.fromFieldRelativeSpeeds(chassisSpeeds,
  // getGyroYaw()));
  ...
  public double getRobotOrientationForSpeaker() {
    double robotsOrientation =
        Math.signum(MathUtil.applyDeadband(getPose().getRotation().getDegrees(), 25));
  ...
  public void updateLimelightReading(DoubleSupplier robotYaw, DoubleSupplier robotYawRate) {
    LimelightHelpers.SetRobotOrientation(
        LIMELIGHT_NAME, robotYaw.getAsDouble(), robotYawRate.getAsDouble(), 0, 0, 0, 0);
    ...
    if (!doRejectUpdate) {
      swerveDrive.addVisionMeasurement(
          mt2.pose, mt2.timestampSeconds, VecBuilder.fill(0.3, 0.3, 9999999));
